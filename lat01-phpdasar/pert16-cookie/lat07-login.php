<?php 
    ...
    if(isset($_COOKIE['id']) && isset($_COOKIE['key'])){
        // Cari data siswa berdasarkan id nya, agar dapat usernamenya
        // Bandingkan usernamenya dengan keynya
        $id = $_COOKIE['id'];
        $key = $_COOKIE['key'];
        ...
        // Cek cookie apakah sama dengan username
    ...
    if(isset($_POST["login"])){
        ...
        if(mysqli_num_rows($result) === 1){
            ...
            // set session login yg akan di cek di setiap file
            $_SESSION["login"] = true;
            ...
            if(isset($_POST['remember'])){
                ...
                // setcookie baru di enkripsi agar aman
                // ambil id dan username user, lalu di hash(sha256)
                // sha adalah algoritma dari hash, ada banyak contoh lainnya di docs
                setcookie('id', $row['id'], time() + 30);
                setcookie('key', hash('sha256', $row['username']) , time() + 30);
                // key adalah samaran dari username agar hacker tidak mengetahui bahwa itu adalah username
